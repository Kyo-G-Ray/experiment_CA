...
# num_cell = 101 # セルの数 
num_cell = 17 # セルの数 
t_transient = 0 # 過渡状態 Ttransient = 1000
...
# cell_tmp_a = [0] * num_cell
cell_list_dp_all = [] # 全世代のセルの情報を記録するリスト
...
if fixedOrNot == 'f':
    if randomOrNot == 'n':
        ...
        save_path = './dp/' + str(ruleNum) + '.eps'
        save_path = './dp/' + str(ruleNum) + '.png'
...
for num in range(5):
  ...
  # file_name = './data/' + str(ruleNum) + '_dp.txt' # (ルール番号).txt
  # file = open(file_name, 'w')

  cell = cell_dp
  # print(math.ceil(num_cell / 2) - 1)
  if cell[math.ceil(num_cell / 2) - 1] == 0:
      cell[math.ceil(num_cell / 2) - 1] = 1
  else:
      cell[math.ceil(num_cell / 2) - 1] = 0
  cell_list_dp_all.clear()
  cell_list_dp_all.append(cell)
      ...
      if t >= t_transient: # 過渡状態後 (1000 to 1099)
          if d_spectrum_sum != 0:
              ...
              if len(h) == d_spectrum_sum:
                  h.append(0)
              h[d_spectrum_sum] += 1 # 異なる密度を数える配列の d_spectrum 番目をインクリメント
          cell_list_dp_all.append(cell) # グラフ出力用
          # file.write(str(cell_list) + '  ' + str(d_spectrum_sum) + '\n')
      d_spectrum_sum = 0 # 初期化
  ...
  # plt.tick_params(labelbottom=False, labelleft=False, bottom=False, left=False)
  # # plt.imshow(cell_list, cmap="binary")
  # plt.imshow(cell_list_dp_all, cmap="binary")
  # plt.savefig(save_path)
  # plt.show()
  # plt.close() #大量のグラフを生成する場合、必要
  ...
  dp_result = [[0] * num_cell for i in range(t_sample)]

  for i in range(t_sample):
      for j in range(num_cell):
          if cell_list_all[i][j] == cell_list_dp_all[i][j]:
              dp_result[i][j] = 0
  ...
  save_path = './dp/' + str(ruleNum) + '_dp_result.png'

  plt.figure()
  plt.tick_params(labelbottom=False, labelleft=False, bottom=False, left=False)
  # plt.imshow(cell_list, cmap="binary")
  plt.imshow(dp_result, cmap="binary")
  plt.savefig(save_path)
  plt.show()
  plt.close() #大量のグラフを生成する場合、必要
