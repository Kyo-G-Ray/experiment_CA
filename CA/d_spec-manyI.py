...
def initialize_i(randomOrNot):
    ...
    # 初期値ランダムの場合
        ...
        cell = np.random.randint(0, 2, num_cell)
        d_spectrum_sum_ini = 0
        for q in range(0, num_cell, 1):
            d_spectrum_sum_ini += cell[q]

        d_ini = d_spectrum_sum_ini/num_cell
        if(d_ini < 0.3 or 0.7 < d_ini):
            while (d_ini < 0.3 or 0.7 < d_ini):
...
for num_i in range(150):

    cell = initialize_i(randomOrNot)
...
print('τ_list = ' + str(τ_list))
print('τ_listの最大値 = ' + str(max(τ_list)))
print('τ_listの最小値 = ' + str(min(τ_list)))
